...
var config = MathJax.Hub.CombineConfig('Reset', {}); // From https://github.com/mathjax/MathJax-node/pull/348#issuecomment-476374035.
...
MathJax.Hub.Register.StartupHook("TeX begingroup Ready", function () {
  ...
  var rootStack = TEX.rootStack,
      FIND = rootStack.Find.bind(rootStack); // Save the original global definitions.

  rootStack.globalFrame = rootStack.stack[0]; // A new (temporary) global definition holder.

  rootStack.stack[0] = NSFRAME();

  rootStack.Find = function (name, type) {
    // Don't let this be redefined.
    if (name === config.resetMacroName && type === "macros") return config.resetMacroName; // Look up the name.
